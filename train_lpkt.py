    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
               ...
               # grads, vars = zip(*optimizer.compute_gradients(lpkt.loss))
                #grads, _ = tf.clip_by_global_norm(grads, clip_norm=FLAGS.norm_ratio)
                #train_op = optimizer.apply_gradients(zip(grads, vars), global_step=lpkt.global_step, name="train_op")
