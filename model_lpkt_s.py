...
class LPKTS(object):
    ...
    def __init__(self, batch_size, num_steps, num_skills, hidden_size):
        ...
        w_f = weight_variable([shape[2],  4*hidden_size], name = 'w_f', training = self.is_training )
        b_f = bias_variable([shape[2]],  name='b_f', training = self.is_training )
        w_f2 = weight_variable([shape[2],  2*hidden_size], name = 'w_f2', training = self.is_training )
        b_f2 = bias_variable([shape[2]],  name='b_f2', training = self.is_training )
        ...
        w_fuse = weight_variable([shape[2], 3*hidden_size], name = 'w_fuse', training = self.is_training )
        b_fuse = bias_variable([shape[2]],  name='b_fuse', training = self.is_training )
        ...
        for i in range(1,self.num_steps+1):
           ...
           # kkk = tf.reduce_mean(knowledge_matrix, axis = 1)
            kkk = tf.matmul(kc_one, knowledge_matrix)
            kkk = tf.squeeze(kkk, 1)

            q = tf.concat([q0, it_one, q1, kkk], axis = -1)
            qq = tf.concat([q0, it_one, q1,student], axis = -1)
            att_score = kc_one  # b, 1, num_skills

            learn_gates = tf.sigmoid(tf.matmul(qq,  tf.transpose(w_l, [1,0])+b_l), name='l_gate')
          ...
          #  c_title = tf.nn.dropout(c_title, self.dropout_keep_prob)
           ...
           # learn_gates = tf.nn.dropout(learn_gates, self.dropout_keep_prob)
            ...
            q_f =  tf.concat([knowledge_matrix, learn_gates_t, its, sts], axis = -1)
        ...
        learning_progress = tf.add(learning_progress, 0, name = 'learning_progress')#
        ...
        sts = tf.expand_dims(student, axis = 1)
        sts = tf.tile(sts, [1,num_steps,1])
        output = tf.matmul(target_kc, output) # b,500,1,hidden_size
        output = tf.reduce_mean(output, axis = 2)


        output = tf.concat([sts,output,target_id], axis = -1)
